...
namespace LLInitParam
{
	...
	void BlockDescriptor::addParam(const ParamDescriptorPtr in_param, const char* char_name)
	{
		...
		else
		{
			...
			mNamedParams.insert_or_assign(name, param);
		}
	...
	bool BaseBlock::validateBlock(bool emit_errors) const
	{
		...
		for (auto it = block_data.mValidationList.begin(); it != block_data.mValidationList.end(); ++it)
		{
	...
	bool BaseBlock::serializeBlock(Parser& parser, Parser::name_stack_t& name_stack, const predicate_rule_t predicate_rule, const LLInitParam::BaseBlock* diff_block) const
	{
		...
		for (const auto& mUnnamedParam : block_data.mUnnamedParams)
        {
			param_handle_t param_handle = mUnnamedParam->mParamHandle;
			const Param* param = getParamFromHandle(param_handle);
			ParamDescriptor::serialize_func_t serialize_func = mUnnamedParam->mSerializeFunc;
		...
		for (const auto& [param_name, param_desc] : block_data.mNamedParams)
		{
			param_handle_t param_handle = param_desc->mParamHandle;
			const Param* param = getParamFromHandle(param_handle);
			ParamDescriptor::serialize_func_t serialize_func = param_desc->mSerializeFunc;
			if (serialize_func && predicate_rule.check(ll_make_predicate(PROVIDED, param->anyProvided())))
			{
				...
				for (const auto& mUnnamedParam : block_data.mUnnamedParams)
                {
					if (param_handle == mUnnamedParam->mParamHandle)
					{
						duplicate = true;
						break;
					}
				...
				name_stack.push_back(std::make_pair(param_name, !duplicate));
				const Param* diff_param = diff_block ? diff_block->getParamFromHandle(param_handle) : NULL;
				serialized |= serialize_func(*param, parser, name_stack, predicate_rule, diff_param);
				name_stack.pop_back();
			}
	...
	bool BaseBlock::inspectBlock(Parser& parser, Parser::name_stack_t name_stack, S32 min_count, S32 max_count) const
	{
		...
		for (const auto& unnamed_param : block_data.mUnnamedParams)
        {
			param_handle_t param_handle = unnamed_param->mParamHandle;
			const Param* param = getParamFromHandle(param_handle);
			ParamDescriptor::inspect_func_t inspect_func = unnamed_param->mInspectFunc;
			if (inspect_func)
			{
				name_stack.push_back(std::make_pair("", true));
				inspect_func(*param, parser, name_stack, unnamed_param->mMinCount, unnamed_param->mMaxCount);
				name_stack.pop_back();
			}
		}
		...
		for(const auto&[param_name, param_desc] : block_data.mNamedParams)
		{
			param_handle_t param_handle = param_desc->mParamHandle;
			const Param* param = getParamFromHandle(param_handle);
			ParamDescriptor::inspect_func_t inspect_func = param_desc->mInspectFunc;
			if (inspect_func)
			{
				// Ensure this param has not already been inspected
				bool duplicate = false;
				for (const auto& param : block_data.mUnnamedParams)
                {
					if (param_handle == param->mParamHandle)
					{
						duplicate = true;
						break;
					}
				...
				name_stack.push_back(std::make_pair(param_name, !duplicate));
				inspect_func(*param, parser, name_stack, param_desc->mMinCount, param_desc->mMaxCount);
				name_stack.pop_back();
			}
		}
	...
	bool BaseBlock::deserializeBlock(Parser& p, Parser::name_stack_range_t& name_stack_range, bool ignored)
	{
		...
		if (names_left)
		{
            ...
            auto found_it = block_data.mNamedParams.find(top_name);
		...
		for (auto& param : block_data.mUnnamedParams)
        {
			Param* paramp = getParamFromHandle(param->mParamHandle);
			ParamDescriptor::deserialize_func_t deserialize_func = param->mDeserializeFunc;
	...
	void BaseBlock::addSynonym(Param& param, const std::string& synonym)
	{
		...
		if (block_data.mInitializationState == BlockDescriptor::INITIALIZING)
		{
			...
			if (param_descriptor)
			{
				...
				{
					block_data.mNamedParams.insert_or_assign(synonym, param_descriptor);
				}
			}
		}
	}
	...
	const std::string& BaseBlock::getParamName(const BlockDescriptor& block_data, const Param* paramp) const
	{
		...
		for (const auto&[param_name, param_ptr] : block_data.mNamedParams)
		{
			if (param_ptr->mParamHandle == handle)
			{
				return param_name;
			}
		}
	...
	ParamDescriptorPtr BaseBlock::findParamDescriptor(const Param& param)
	{
		...
		for (auto& param : descriptor.mAllParams)
        {
			if (param->mParamHandle == handle) return param;
		}
