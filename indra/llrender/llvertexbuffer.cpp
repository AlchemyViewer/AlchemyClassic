...
void LLVertexBuffer::drawArrays(U32 mode, const std::vector<LLVector3>& pos, const std::vector<LLVector3>& norm)
{
	...
	if (shader)
	{
		glVertexAttribPointer(TYPE_VERTEX, 3, GL_FLOAT, GL_FALSE, 0, pos[0].mV);
		glVertexAttribPointer(TYPE_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, norm[0].mV);
	}
	else
	{
...
void LLVertexBuffer::drawElements(U32 mode, const LLVector4a* pos, const LLVector2* tc, S32 num_indices, const U16* indicesp)
{
	...
	if (LLGLSLShader::sNoFixedFunction)
	{
		glVertexAttribPointer(TYPE_VERTEX, 3, GL_FLOAT, GL_FALSE, 16, pos);
		if (tc)
		{
			glVertexAttribPointer(TYPE_TEXCOORD0, 2, GL_FLOAT, GL_FALSE, 0, tc);
		}
	}
	else
	{
...
void LLVertexBuffer::setupVertexArray()
{
	...
	for (U32 i = 0; i < TYPE_MAX; ++i)
	{
		if (mTypeMask & (1 << i))
		{
			...
			{
				glVertexAttribPointer(i, attrib_size[i], attrib_type[i], attrib_normalized[i], sTypeSize[i], reinterpret_cast<void*>(mOffsets[i])); 
			}
		}
		else
		{
