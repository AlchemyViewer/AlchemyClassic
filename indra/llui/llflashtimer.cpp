    ...
    mUnset(false)
{
	...
	static LLUICachedControl<S32> sFlashCount("FlashCount", 8);
	mFlashCount = 2 * ((count > 0) ? count : sFlashCount);
	if (mPeriod <= 0)
	{
		static LLUICachedControl<F32> sFlashPeriod("FlashPeriod", 0.5f);
		mPeriod = sFlashPeriod;
	}
}
