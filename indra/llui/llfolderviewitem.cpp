...
void LLFolderViewItem::refresh()
{
	...
	if (mRoot->useLabelSuffix())
	{
		...
		mLabelSuffix = utf8string_to_wstring(vmi.getLabelSuffix());
	}
...
S32 LLFolderViewItem::arrange( S32* width, S32* height )
{
	...
	if (mLabelWidthDirty)
	{
		mLabelWidth = getLabelXPos() + getLabelFontForStyle(mLabelStyle)->getWidth(mLabel.getWString().c_str()) + getLabelFontForStyle(mLabelStyle)->getWidth(mLabelSuffix.c_str()) + mLabelPaddingRight; 
		mLabelWidthDirty = false;
	}
...
void LLFolderViewItem::draw()
{
	...
	LLWString combined_string = mLabel.getWString() + mLabelSuffix;
    ...
    if (filter_string_length > 0)
    {
        F32 match_string_left = text_left + font->getWidthF32(combined_string.c_str(), 0, mViewModelItem->getFilterStringOffset());
        F32 yy = (F32)getRect().getHeight() - font->getLineHeight() - (F32)mTextPad - (F32)TOP_PAD;
        font->render( combined_string, mViewModelItem->getFilterStringOffset(), match_string_left, yy,
