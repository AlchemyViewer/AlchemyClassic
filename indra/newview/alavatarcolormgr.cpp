...
void ALAvatarColorMgr::addOrUpdateCustomColor(const LLUUID& id, e_custom_colors color_val)
{
	...
	{
	case E_FIRST_COLOR:
		color = avatar_color_1.get();
		break;
	case E_SECOND_COLOR:
		color = avatar_color_2.get();
		break;
	case E_THIRD_COLOR:
		color = avatar_color_3.get();
		break;
	case E_FOURTH_COLOR:
		color = avatar_color_4.get();
		break;
	default:
	...
	{
		mCustomColors.insert(uuid_color_umap_t::value_type(id, color));
	}
}
