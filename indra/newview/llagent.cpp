...
void LLAgent::startTyping()
{
	...
	LLChatUtilities::sendChatFromViewer("", CHAT_TYPE_START, FALSE);
}
...
void LLAgent::stopTyping()
{
	if (mRenderState & AGENT_STATE_TYPING)
	{
		...
		LLChatUtilities::sendChatFromViewer("", CHAT_TYPE_STOP, FALSE);
	}
}
...
void LLAgent::teleportViaLocationLookAt(const LLVector3d& pos_global, const LLVector3& look_at)
{
	...
	mTeleportRequest = std::static_pointer_cast<LLTeleportRequest>(std::make_shared<LLTeleportRequestViaLocationLookAt>(pos_global, (look_at.isExactlyZero()) ? LLViewerCamera::getInstance()->getAtAxis() : look_at));
	startTeleportRequest();
}
// [/RLVa:KB]
...
void LLAgent::requestAgentUserInfoCoro(std::string capurl)
{
	...
	auto httpAdapter = std::make_shared<LLCoreHttpUtil::HttpCoroutineAdapter>("requestAgentUserInfoCoro", httpPolicy);
