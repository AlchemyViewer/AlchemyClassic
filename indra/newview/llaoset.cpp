...
LLAOSet::LLAOSet(const LLUUID& inventoryID)
...
LLAOSet::AOState* LLAOSet::getState(S32 name)
{
	return &mStates[name];
}

LLAOSet::AOState* LLAOSet::getStateByName(const std::string& name)
{
	for (S32 index = 0; index < AOSTATES_MAX; ++index)
	{
...
void LLAOSet::startTimer(F32 timeout)
{
	mEventTimer.stop();
	mPeriod = timeout;
	mEventTimer.start();
...
BOOL LLAOSet::tick()
{
	LLAOEngine::instance().cycleTimeout(this);
	return FALSE;
}
