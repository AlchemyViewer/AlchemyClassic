...
void ExperienceAssociationResponder::fetchAssociatedExperience(LLSD& request, callback_t callback)
{
    ...
    if (!object)
    {
        LL_DEBUGS() << "Object with ID " << request["object-id"] << " not found via gObjectList.findObject() in fetchAssociatedExperience\n";
        LL_CONT << "Using gAgent.getRegion() instead of object->getRegion()" << LL_ENDL;
    }
    LLViewerRegion* region = object ? object->getRegion() : gAgent.getRegion();
    if (region)
    {
