...
void LLMaterialMgr::onGetResponse(bool success, const LLSD& content, const LLUUID& region_id)
{
	...
	LLSD response_data;
	U32 uzip_result = LLUZipHelper::unzip_llsd(response_data, const_cast<U8*>(content_binary.data()), content_binary.size());
...
void LLMaterialMgr::processGetAllQueueCoro(LLUUID regionId)
{
	...
	auto httpAdapter = std::make_shared<LLCoreHttpUtil::HttpCoroutineAdapter>("processGetAllQueue", LLCore::HttpRequest::DEFAULT_POLICY_ID);
