...
BOOL LLViewerWindow::handleKey(KEY key, MASK mask)
{
	...
	static LLCachedControl<S32> letter_focus_chat_bar(gSavedSettings, "LetterKeysFocusChatBar");
	if (letter_focus_chat_bar && !gAgentCamera.cameraMouselook() &&
...
void LLViewerWindow::updateUI()
{
	...
	{	
			...
			&& !mWindow->isCursorHidden())
		{
			...
			static LLCachedControl<bool> debug_show_xui_names(gSavedSettings, "DebugShowXUINames", false);
	...
	BOOL keep_window_aspect, BOOL is_texture, BOOL show_ui, BOOL do_rebuild, LLSnapshotModel::ESnapshotLayerType type, S32 max_size)
{
	...
	{	
		...
		{
			const U32 color_fmt = type == LLSnapshotModel::SNAPSHOT_TYPE_DEPTH ? GL_DEPTH_COMPONENT : GL_RGBA;
