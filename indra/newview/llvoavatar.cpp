...
void dump_visual_param(llofstream& ofstream, LLVisualParam* viewer_param, F32 value)
{
	...
	ofstream << llformat("\t\t<param id=\"%d\" name=\"%s\" display=\"%s\" value=\"%.3f\" u8=\"%d\" type=\"%s\" wearable=\"%s\" group=\"%d\"/>\n",
...
void LLVOAvatar::dumpArchetypeXML(const std::string& prefix, bool group_by_wearables )
{
	...
	else
	{
             ...
             name_iter != attach_names.end(); ++name_iter)
        {
			...
			const LLVector3& pos = pJoint->getPosition();
			const LLVector3& scale = pJoint->getScale();
			outstream << llformat("\t\t<attachment_point name=\"%s\" position=\"%f %f %f\" scale=\"%f %f %f\"/>\n",
							 pJoint->getName().c_str(), pos[0], pos[1], pos[2], scale[0], scale[1], scale[2]);
        }
             ...
             name_iter != all_names.end(); ++name_iter)
        {
			...
			if (pJoint && pJoint->hasAttachmentPosOverride(pos,mesh_id))
			{
				...
				outstream << llformat("\t\t<joint_offset name=\"%s\" position=\"%f %f %f\" mesh_id=\"%s\" count=\"%d\" distinct=\"%d\"/>\n",
             ...
             name_iter != all_names.end(); ++name_iter)
        {
			...
			if (pJoint && pJoint->hasAttachmentScaleOverride(scale,mesh_id))
			{
				...
				outstream << llformat("\t\t<joint_scale name=\"%s\" scale=\"%f %f %f\" mesh_id=\"%s\" count=\"%d\" distinct=\"%d\"/>\n",
...
{/*
	...
	for (j = 0; j < buffer->getNumVerts(); ++j)
	{
...
void LLVOAvatar::calculateUpdateRenderComplexity()
{
	...
	if (mVisualComplexityStale)
	{
		...
		if (isSelf())
		{
		    ...
		    for (LLAvatarAppearanceDictionary::Textures::const_iterator iter = avatar_dictionary.getTextures().begin(),
				 end_iter = avatar_dictionary.getTextures().end();
				 iter != end_iter;
				 ++iter)
			{
			    const LLAvatarAppearanceDictionary::TextureEntry *texture_dict = iter->second;
