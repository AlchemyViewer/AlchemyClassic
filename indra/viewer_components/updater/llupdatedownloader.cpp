...
void LLUpdateDownloader::Implementation::initializeCurlGet(std::string const & url, bool processHeader)
{
	...
	throwOnCurlError(curl_easy_setopt(mCurl, CURLOPT_XFERINFOFUNCTION, &xferinfo_callback));
	throwOnCurlError(curl_easy_setopt(mCurl, CURLOPT_XFERINFODATA, this));
	throwOnCurlError(curl_easy_setopt(mCurl, CURLOPT_NOPROGRESS, 0));
	...
	throwOnCurlError(curl_easy_setopt(mCurl, CURLOPT_CAINFO, gDirUtilp->getCAFile().c_str()));
	throwOnCurlError(curl_easy_setopt(mCurl, CURLOPT_SSL_VERIFYHOST, 2));
	throwOnCurlError(curl_easy_setopt(mCurl, CURLOPT_SSL_VERIFYPEER, 1));
